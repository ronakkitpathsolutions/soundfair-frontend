...
const EditTipsForm: FC<PageProps> = ({ ...props }: PageProps) => {
  ...
  useEffect(() => {
    ...
    dispatch(
      ...
      setFormData({
        ...formData,
        session_type: session.type || 'Reading',
        type: payload?.type || 'text',
        description: payload?.description || 'text',
        question_title: payload?.question_title || '',
        isTroubleShootChecked:
          !!payload?.troubleshoot_title ||
          !!payload?.troubleshoot_info_title ||
          !!payload?.troubleshoot_description,
        troubleshoot_title: payload?.troubleshoot_title || '',
        troubleshoot_info_title: payload?.troubleshoot_info_title || '',
        troubleshoot_description: payload?.troubleshoot_description || '',
      })
      setOptions(
        payload?.options
          ? JSON.parse(payload?.options.replace(/\\"/g, '"')?.slice(1, -1))
          : [''],
      )
    })
  ...
  const handleSubmit = async (values: FormValues) => {
    ...
    try {
      const payload = {
        type: values.type,
        tip_id: String(router.query.tip_id),
      ...
      const response = await api.tips.update(payload)
      if (response?.data) {
        setIsLoading(false)
      ...
      <div className="w-full pb-8">
        <div className="flex w-auto items-center text-sm">
          ...
          <span
            ...
            onClick={() =>
              router.push(
                `/categories/${router.query.modules_id}/view/${router.query.id}/session/${router.query.session_id}/view/${router.query.tip_id}`,
              )
            }
          >
            {formData.question_title || formData.description}
          </span>
          <span className="mx-[2px]">
            <ChevronRight className="h-4 w-4" />
          </span>
        ...
        <div className="mt-8">
          <h4 className="text-display-sm font-medium">Edit Tip</h4>
          <div className="mt-4">
            <Form
              {...{
                isDisabled,
